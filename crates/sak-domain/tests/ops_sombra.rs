//! Fase 5.2 — gob.entrar_sombra + gob.estado_sombra (sin activar época).
...
fn hasta_firmada(st: &mut EstadoOps, ident: &str) -> String {
...
#[test]
fn sombra_ok_y_estado_lectura() {
    let mut st = EstadoOps::en_memoria().unwrap();
    let hash = hasta_firmada(&mut st, "N-SOMBRA-1");
    let desde = 1_000u64;
    let r = despachar_con_estado(
        &format!(
            r#"{{"op":"gob.entrar_sombra","req_id":"s","schema_v":1,"hash_paquete":"{hash}","identidad":"op-sombra","rol":"gobernanza-sombra","epoca_vista":"3","confirmacion_independiente":true,"ahora_ms":"{desde}"}}"#
        ),
        Some(&mut st),
    );
    assert_eq!(r.resultado, "OK", "{}", r.a_json());
    assert!(r.cuerpo.contains("EN_SOMBRA"));
    assert!(r.cuerpo.contains("evalua_sin_aplicar\":true"));
    assert!(r.cuerpo.contains("aplica_como_vivo\":false"));
    assert!(r.cuerpo.contains("epoca_activada\":false"));
    assert!(r.cuerpo.contains("no_activa\":true"));
    assert!(r.cuerpo.contains("no_aplica_como_vivo\":true"));
    assert!(r.cuerpo.contains("no_certifica_conformidad\":true"));
    assert!(r.cuerpo.contains("anti_engano"));
    assert!(r.cuerpo.contains(&format!("ventana_sombra_ms\":{VENTANA_SOMBRA_MS}")));
    assert!(st.gob.hash_activo().is_none());

    let mid = desde + VENTANA_SOMBRA_MS / 2;
    let e = despachar_con_estado(
        &format!(
            r#"{{"op":"gob.estado_sombra","req_id":"e","schema_v":1,"hash_paquete":"{hash}","ahora_ms":"{mid}"}}"#
        ),
        Some(&mut st),
    );
    assert_eq!(e.resultado, "OK", "{}", e.a_json());
    assert!(e.cuerpo.contains("en_sombra\":true"));
    assert!(e.cuerpo.contains("evalua_sin_aplicar\":true"));
    assert!(e.cuerpo.contains("aplica_como_vivo\":false"));
    assert!(e.cuerpo.contains("ventana_completa\":false"));
    assert!(e.limites.iter().any(|l| *l == "LECTURA"));

    let fin = desde + VENTANA_SOMBRA_MS;
    let e2 = despachar_con_estado(
        &format!(
            r#"{{"op":"gob.estado_sombra","req_id":"e2","schema_v":1,"hash_paquete":"{hash}","ahora_ms":"{fin}"}}"#
        ),
        Some(&mut st),
    );
    assert_eq!(e2.resultado, "OK");
    assert!(e2.cuerpo.contains("ventana_completa\":true"));
    assert!(e2.cuerpo.contains("epoca_activada\":false"));
}

#[test]
fn sombra_sin_confirmacion_deny() {
    let mut st = EstadoOps::en_memoria().unwrap();
    let hash = hasta_firmada(&mut st, "N-SOMBRA-2");
    let r = despachar_con_estado(
        &format!(
            r#"{{"op":"gob.entrar_sombra","req_id":"s","schema_v":1,"hash_paquete":"{hash}","identidad":"op","rol":"gobernanza-sombra","epoca_vista":"1","confirmacion_independiente":false}}"#
        ),
        Some(&mut st),
    );
...
#[test]
fn sombra_sin_doble_firma_deny() {
    let mut st = EstadoOps::en_memoria().unwrap();
    let dig = sha384_dominio(dominio::GOBERNANZA, b"interpretacion operativa borrador MVP");
    let (hash_hex, par_prop, par_ap, _) = hash_y_pares("N-SOMBRA-3", "instrumento-mvp-art-1", dig);
    let raw = body_proponer(
        "N-SOMBRA-3",
        "instrumento-mvp-art-1",
        dig,
    ...
    let r = despachar_con_estado(
        &format!(
            r#"{{"op":"gob.entrar_sombra","req_id":"s","schema_v":1,"hash_paquete":"{hash_hex}","identidad":"op","rol":"gobernanza-sombra","epoca_vista":"1","confirmacion_independiente":true}}"#
        ),
        Some(&mut st),
    );
...
#[test]
fn sombra_con_activar_en_payload_deny() {
    let mut st = EstadoOps::en_memoria().unwrap();
    let hash = hasta_firmada(&mut st, "N-SOMBRA-4");
    let r = despachar_con_estado(
        &format!(
            r#"{{"op":"gob.entrar_sombra","req_id":"s","schema_v":1,"hash_paquete":"{hash}","identidad":"op","rol":"gobernanza-sombra","epoca_vista":"1","confirmacion_independiente":true,"activar_epoca":true}}"#
        ),
        Some(&mut st),
    );
...
#[test]
fn activar_y_revocar_siguen_deny_fijo() {
    // Fase 5.4: sombra/activar/revocar/revertir en allowlist.
    assert!(!es_deny_fijo_ops("gob.entrar_sombra"));
    assert!(!es_deny_fijo_ops("gob.estado_sombra"));
